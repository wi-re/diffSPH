...
__global__ void scatterAdd_kernel(
    ...
    float outputBuffer = 0.f;
    auto offset = neighborOffset[i];
    auto numNeighbor = numNeighbors[i];
    for (int j = 0; j < numNeighbor; j++) {
        outputBuffer += input[offset + j];
    }
    output[i] = outputBuffer;

    // scatterAdd(i, output, input, numParticles, numNeighbors, neighborOffset);
}
...
void scatterAdd_cu(
    ...
    int32_t* numNeighbors,
    int32_t* neighborOffset){
        launchKernel(scatterAdd_kernel, 
            numParticles, 
            output,
            input,
