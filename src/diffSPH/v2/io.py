...
def writeFrame(simulationDataGroup, perennialState, priorState, frameStatistics, config):
    ...
    frameGroup.create_dataset('fluidPosition', data = priorState['fluidPositions'].detach().cpu().numpy())
    frameGroup.create_dataset('fluidVelocity', data = priorState['fluidVelocities'].detach().cpu().numpy())
    frameGroup.create_dataset('fluidDensity', data = priorState['fluidDensities'].detach().cpu().numpy() / config['fluid']['rho0'])
    if config['gravity']['active']:
        frameGroup.create_dataset('fluidGravity', data = priorState['fluidGravityAccel'].detach().cpu().numpy())
    ...
    frameGroup.create_dataset('fluidShiftAmount', data = perennialState['fluidShiftAmount'].detach().cpu().numpy())
    frameGroup.create_dataset('UID', data = perennialState['fluidIndex'].detach().cpu().numpy())
